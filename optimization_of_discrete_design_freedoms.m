...
subplot(1,2,1)
for B = 3:5
    Costs = zeros(Lh+Lg, 1); % kind of silly to consider tau+1>Lh; just flip one to get another
    for tau = 1 : length(Costs)
        try
            ...
            best_fb = fb;
            for num_trial = 1 : 100
                fb.h = rand(Lh,1);   fb.g = rand(Lg,1); % rand seems better than fbd_random_initial_guess :(
